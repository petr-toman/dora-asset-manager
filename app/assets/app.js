...
function initCy(elements) {
    ...
    cy = cytoscape({
        ...
        style: [
            ...
            { selector: 'node[type="hardware"]', style: { 'background-color': '#263244', 'border-color': '#94a3b8' }},
            { selector: 'node[type="software"]', style: { 'background-color': '#0f2f53', 'border-color': '#38bdf8' }},
            { selector: 'node[type="data"]', style: { 'background-color': '#123c2b', 'border-color': '#22c55e' }},
            { selector: 'node[type="process"]', style: { 'background-color': '#452f11', 'border-color': '#f59e0b' }},
            { selector: 'node[type="business_function"]', style: { 'background-color': '#4a2411', 'border-color': '#fb923c' }},
            { selector: 'node[type="supplier"], node[type="provider"], node[type="manufacturer"]', style: { 'background-color': '#30204f', 'border-color': '#a78bfa' }},
            { selector: 'node[type="network"], node[type="location"], node[type="ict_service"], node[type="documentation"]', style: { 'background-color': '#193243', 'border-color': '#67e8f9' }},
            { selector: 'node[criticality="low"]', style: { 'border-width': 2 }},
            { selector: 'node[criticality="medium"]', style: { 'border-width': 3 }},
            { selector: 'node[criticality="high"]', style: { 'border-width': 4, 'shadow-color': '#f59e0b', 'shadow-opacity': .24 }},
            { selector: 'node[criticality="critical"]', style: { 'border-width': 5, 'shadow-color': '#ef4444', 'shadow-opacity': .36 }},
            { selector: 'edge', style: {
                'curve-style': 'bezier',
            ...
            { selector: 'edge[type="contains"]', style: { 'line-style': 'dashed', 'line-color': '#7dd3fc', 'target-arrow-color': '#7dd3fc' }},
            { selector: 'edge[type="processes_data"], edge[type="uses_data"]', style: { 'line-color': '#22c55e', 'target-arrow-color': '#22c55e' }},
            { selector: 'edge[type="supports_process"], edge[type="supports_function"]', style: { 'line-color': '#f59e0b', 'target-arrow-color': '#f59e0b' }},
            { selector: 'edge[type="provided_by"], edge[type="supplied_by"], edge[type="manufactured_by"]', style: { 'line-color': '#a78bfa', 'target-arrow-color': '#a78bfa' }},
            { selector: 'edge[type="hosts"], edge[type="runs_on"]', style: { 'line-color': '#38bdf8', 'target-arrow-color': '#38bdf8' }},
