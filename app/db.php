...
function seed_demo_data(PDO $pdo): void
{
    ...
    $stmt = $pdo->prepare('INSERT INTO nodes (type, name, description, criticality, environment, confidentiality, integrity_level, availability, data_sensitivity, status, lifecycle_state, rto_hours, rpo_hours, mtd_hours, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    foreach ($nodes as $n) {
        $stmt->execute([$n[0], $n[1], $n[2], $n[3], $n[4], 'high', 'high', 'high', $n[0] === 'data' ? 'private' : null, 'active', 'production', $n[3] === 'critical' ? 8 : 24, $n[3] === 'critical' ? 24 : 48, $n[3] === 'critical' ? 48 : 72, $now, $now]);
    }
